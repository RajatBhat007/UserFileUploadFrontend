...
export class UserUploadComponent implements OnInit {
  ...
  constructor(public http: FileUploadService, private renderer: Renderer2,private cdr: ChangeDetectorRef,private sanitizer: DomSanitizer) { }
  ...
  public triggerObservableVideoRecording: Observable<void> = this.trigger.asObservable();
...
uploadImage() {
  ...
  const file = new File([blob], `captured_image_${new Date().getTime()}.png`, { type: 'image/png' });
  console.log(file)
  this.file=file;
  ...
  .catch(error => {
    ...
    if (this.startRecordingBtn) this.startRecordingBtn.addEventListener('click', () => this.startRecording());
    if (this.stopRecordingBtn) this.stopRecordingBtn.addEventListener('click', () => this.stopRecording());
    // if (this.cancelBtn) this.cancelBtn.addEventListener('click', () => this.cancelCamera());
    if (this.uploadBtn) this.uploadBtn.addEventListener('click', () => this.uploadVideo());
  }
